...
with open('data/p53_delta_files.txt') as f:
    wsis = [im.rstrip() for im in f]

slide_path = '/media/prew01/DELTA/slides/p53'
...
for image in wsis:
    ...
    output = os.path.join('data/delta_tissue/p53', image)
    ...
    try:
        if os.path.isfile(output + '.pml'):
            ...
            slidl_slide.save(fileName=image, folder='data/delta_tissue/p53')
...
df.to_csv('p53_delta_tissue_patches.csv')
print(len(df))