...
tile_size = 392
tile_overlap = 0.0
model_state_dict_path = '/home/prew01/repos/slide-classifier/models/deep-tissue-detector_densenet_state-dict.pt'
tissue_threshold = 0.99

with open('he_files.txt') as f:
    wsis = [im.rstrip() for im in f]

slide_path = '/media/prew01/DELTA/slides/he'
...
for image in wsis:
    ...
    output = os.path.join('delta_tissue', image)
    ...
    try:
        if os.path.isfile(output + '.pml'):
            ...
            slidl_slide.save(fileName=image, folder='delta_tissue')
...
df.to_csv('he_delta_tissue_patches.csv')
print(len(df))