...
def parse_args():
    ...
    parser.add_argument('--hed_contrast', type=float, default=2, help='contrast increase; if the tissue is not well separated from the background, increase this value; you can easily go up to 20 or higher')
    ...
    parser.add_argument('--filter_kernel', default=5, help='filter kernel for black and red pen markers, default=5 leads to kernel of (5,5)')
...
def get_mask(file_name, hed_contrast=2, gray_contrast=10, gray_threshold=220, min_width=1200,
             ...
             red_marker=False, black_marker=False, green_marker=False, hed=True, gray=True, kernel=(5,5), dilute=True):
    ...
    # dilute pen markers
    if dilute:
        selem = square(int(min_width/350)) #TODO
...
if __name__ == '__main__':
    ...
    for file in tqdm(files):
        ...
        get_mask(file, hed_contrast=hed_contrast, gray_contrast=gray_contrast, gray_threshold=gray_threshold, min_width=min_width,
             resize=resize, save=save_path, min_pixel_count=10, blue_marker=blue_marker,
             red_marker=red_marker, black_marker=black_marker, green_marker=green_marker, hed=he, gray=gray, kernel=(filter_kernel,filter_kernel), dilute=dilute)
    end_time = time.time()
