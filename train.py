...
def parse_args():
	...
	parser.add_argument("--optim", tpye=str, default='Adam', help='Choose Optimiser to use between SGD/Adam, default is Adam')
...
def train_model(model, params, device, criterion, optimizer, scheduler, dataloaders, num_epochs=25, silent=False):
	...
	for epoch in range(num_epochs):
		...
		for phase in ['train', 'val']:
			...
			stats[phase].append({
					str(phase)+'/epoch': epoch,
					str(phase)+'/loss': epoch_loss, 
					str(phase)+'/accuracy': epoch_acc, 
					str(phase)+'/precision': epoch_prec, 
					str(phase)+'/recall': epoch_rec,
					str(phase)+'/weighted_accuracy': epoch_weighted_acc,
					str(phase)+'/weighted_precision': epoch_weighted_prec,
					str(phase)+'/weighted_recall': epoch_weighted_rec,
					str(phase)+'/weighted_f1': epoch_weighted_f1})
...
if __name__ == '__main__':
	...
	num_train_cases = round(args.split * len(cases))
	...
	for case in cases[-1 - num_train_cases:-1]:
		for class_idx, class_name in enumerate(class_names):
			path, dirs, files = next(
				os.walk(os.path.join(data_dir, case, class_name)))
	...
	data_transforms = image_transforms(params['patch_size'], channel_means, channel_stds, args.colourjitter)
